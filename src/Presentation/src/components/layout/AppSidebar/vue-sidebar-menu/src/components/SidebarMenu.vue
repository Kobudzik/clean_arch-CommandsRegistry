<template>
    <div class="v-sidebar-menu" :class="sidebarClass" :style="[{ 'max-width': sidebarWidth }]" @mouseleave="onMouseLeave" @mouseenter="onMouseEnter">
        ...
        <slot name="footer" />
        <button v-if="!hideToggle" class="vsm--toggle-btn" :class="{ 'vsm--toggle-btn_slot': $slots['toggle-icon'] }" @click="onToggleClick">
...
export default {
    ...
    computed: {
        ...
        sidebarClass() {
            return [!this.isCollapsed ? "vsm_expanded" : "vsm_collapsed", this.theme ? `vsm_${this.theme}` : "", this.rtl ? "vsm_rtl" : "", this.relative ? "vsm_relative" : ""];
        },
        mobileItemStyle() {
            return {
                item: [
